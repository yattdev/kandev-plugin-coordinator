...
describe("CoordinatorClient", () => {
  it("uses declared authenticated actions without placing workspace authority in the body", async () => {
    ...
    await client.run("cycle", "manual-1");
		await client.policy();
		await client.savePolicy([{ workflow_id: "workflow-1", workstep_id: "step-1" }]);
		...
		expect(invokeAction.mock.calls).toEqual([
			...
			["coordinator.policy", { workspaceId: "workspace-1" }, { signal: undefined }],
			["coordinator.policy", { workspaceId: "workspace-1", body: { selections: [{ workflow_id: "workflow-1", workstep_id: "step-1" }] } }, { signal: undefined }],
		]);
  });
});
