...
func (h *actionFakeHost) GetState(_ context.Context, scope, id, key string) (map[string]any, bool, error) {
	value, found := h.state[actionStateKey(scope, id, key)]
	return value, found, nil
}
func (h *actionFakeHost) SetState(_ context.Context, scope, id, key string, value map[string]any) error {
	h.state[actionStateKey(scope, id, key)] = value
	return nil
}
func (*actionFakeHost) DeleteState(context.Context, string, string, string) error { return nil }
func (*actionFakeHost) ListState(context.Context, string, string) ([]pluginsdk.StateEntry, error) {
	return nil, nil
}
...
func (*actionFakeHost) GetSecret(context.Context, string) (string, bool, error) {
	return "", false, nil
}
...
func TestReportActionStoresOnlyInVerifiedWorkspace(t *testing.T) {
	...
	require.Equal(t, "All clear", value["report"])
}
...
func TestWorkflowPolicyActionPersistsPerWorkstepPrompts(t *testing.T) {
	...
	require.Equal(t, true, decoded["configured"])
}
