...
const ( statusAction = "coordinator.status"; reportAction = "coordinator.report"; policyAction = "coordinator.workflow-policy"; statusTool = "coordinator_status"; reportTool = "coordinator_record_report"; coordinatorStateKey = "coordinator_state" )
...
func (p *coordinatorPlugin) handleWorkflowPolicy(ctx context.Context, workspaceID string, body []byte) (*pluginsdk.PluginActionResponse, error) {
	...
	if err := json.Unmarshal(body, &input); err != nil { return nil, fmt.Errorf("coordinator: decoding workflow policy: %w", err) }
	...
	if input.Operation != "save" { return nil, fmt.Errorf("coordinator: workflow policy operation must be get or save") }
	policy := WorkflowPolicy{WorkflowID: input.WorkflowID, Worksteps: input.Worksteps}
	if err := p.saveWorkflowPolicy(ctx, workspaceID, policy); err != nil { return nil, err }
